...
function GetFen(){
    ...
    if (g_passingSquare == -1) {
        result += '-';
    }
    else {
        result += FormatSquare(g_passingSquare);
    }
...
var minMateBuffer = minEval + 20;
var maxMateBuffer = maxEval - 20;
...
function InitializeFromFen(fen) {
	...
	g_passingSquare = MakeSquare(row, col);
    }
...
function MakeMove(move){
    ...
    g_moveUndoStack[g_moveCount] = new UndoHistory(g_passingSquare, g_castleRights, g_inCheck, g_baseEval, g_hashKeyLow, g_hashKeyHigh, g_move50, captured);
    g_moveCount++;
    ...
    if (captured) {
        ...
        if (diff < 0) diff = -diff;
        if (diff > 16) {
            g_passingSquare = me ? (to + 0x10) : (to - 0x10);
        }
        g_move50 = 0;
    }
...
function UnmakeMove(move){
    ...
    g_moveCount--;
    g_passingSquare = g_moveUndoStack[g_moveCount].ep;
...
function GenerateCaptureMoves(moveStack, moveScores) {
    ...
    if (g_passingSquare != -1) {
        ...
        var from = g_passingSquare - (inc + 1);
        if ((g_board[from] & 0xF) == pawn) {
            moveStack[moveStack.length] = GenerateMove(from, g_passingSquare, moveflagEPC);
        }

        from = g_passingSquare - (inc - 1);
        if ((g_board[from] & 0xF) == pawn) {
            moveStack[moveStack.length] = GenerateMove(from, g_passingSquare, moveflagEPC);
        }
    }
